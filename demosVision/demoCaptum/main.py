...
def main():
    ...
    model = models.resnet18(
        weights=models.ResNet18_Weights.IMAGENET1K_V1).eval()

    # Compute and visualize attribution for the dog class
    print("Calculating attribution for dog")
    attribution_dog = compute_attribution(
        model, input_img, target=DEFAULT_DOG_TARGET)
    ...
    attribution_cat = compute_attribution(
        model, input_img, target=DEFAULT_CAT_TARGET)
