"""
This module defines several neural network architectures implemented using PyTorch.

Classes:
    - STNNet: Implements a Spatial Transformer Network (STN) for spatial alignment
      of images before classification.
    - SuperResolutionNet: A Super-Resolution Convolutional Neural Network (SRCNN) for
      increasing the resolution of input images using sub-pixel convolution layers.
    - ClassificationNet: A flexible classification network with customizable convolutional
      and fully connected layers for classification tasks.
    - LeNet: A convolutional neural network inspired by the classic LeNet architecture,
      designed for small image classification tasks.

Each model includes clear methods for initialization, forward propagation, and necessary
helper functions for building and training the networks.
"""
import torch
