...
class Config:
    ...
    def load_from_json(self, config_file: str):
        ...
        try:
            ...
            print(f"Error: Config file '{config_file}' is not a valid JSON. Using default values.")
    ...
    def _validate_config(self):
        ...
        if not (isinstance(self.test_batch_size, int) and self.test_batch_size > 0):
            raise ValueError("Test batch size must be a positive integer.")
        if not (isinstance(self.epochs, int) and self.epochs > 0):
            raise ValueError("Epochs must be a positive integer.")
        if not (isinstance(self.learning_rate, float) and self.learning_rate > 0):
            raise ValueError("Learning rate must be a positive float.")
        if not (isinstance(self.learning_rate_gamma, float) and 0 < self.learning_rate_gamma <= 1):
            raise ValueError("Learning rate gamma must be a float between 0 and 1.")
    ...
    def __repr__(self):
        ...
        return (f"Config(batch_size={self.batch_size}, test_batch_size={self.test_batch_size}, "
                ...
                f"learning_rate_gamma={self.learning_rate_gamma}, dry_run={self.dry_run}, "
