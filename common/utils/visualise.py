...
def display_image(image, title=None, mean=None, std=None,
                  skip_show=False, **imshow_kwargs):
    """
...
def display_grid(images, title=None, row_title=None,
                 num_cols=None, **imshow_kwargs):
    """
    ...
    if not isinstance(images[0], list):
        # If images is a 1D list, create a 2D list with specified number of
        # columns
        if num_cols is None:
            # Default to one row if no num_cols is given
            num_cols = len(images)
        images = [images[i:i + num_cols]
